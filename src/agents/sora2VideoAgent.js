...
function writeArtifacts(results, report, artifactContext) {
  ...
  writeAgentQaSummary(
    {
      agentKey: 'sora2VideoAgent',
      agentName: 'Fallback Video Adapter',
      status: report.failedCount > 0 ? 'warn' : 'pass',
      headline:
        report.failedCount > 0
          ? `Fallback Video 已生成 ${report.generatedCount} 个镜头，另有 ${report.failedCount} 个镜头待 fallback`
          : `Fallback Video 已生成 ${report.generatedCount} 个动态镜头`,
      summary:
        report.failedCount > 0
          ? '部分镜头未成功生成，后续可回退到静图合成主链。'
          : '当前所有已路由到 fallback video 的镜头都已成功生成。',
  ...
  for (const shotPackage of shotPackages) {
    ...
    try {
      ...
      results.push({
        ...
        attemptedModels: normalizedError.attemptedModels,
      });
    }
  }
